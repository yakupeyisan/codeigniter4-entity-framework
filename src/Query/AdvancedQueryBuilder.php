...
class AdvancedQueryBuilder
{
    ...
    private function prepareFilterBuilderForCountAggregate(array $additionalJoinNavigationPaths = [])
    {
        ...
        foreach ($this->wheres as $whereItem) {
            ...
            if (is_string($rawSql) && str_contains($rawSql, '.')) {
                if (preg_match_all('/\b([A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)+)\b/', $rawSql, $rawMatches)) {
                    foreach (array_unique($rawMatches[1]) as $dotPath) {
                        foreach ($this->collectReferenceNavigationPathsFromDotPath($dotPath) as $path) {
                            if (! in_array($path, $allNavigationPaths, true)) {
        ...
        foreach ($this->wheres as $index => $whereItem) {
            ...
            if (is_string($whereToApply)) {
                $whereToApply = $this->expandRawSqlNavigationWhereForCount($whereToApply, $builder);
                $this->applyWhere($builder, $whereToApply, $isOr);
            } else {
                $paths = $this->detectNavigationPaths($whereToApply);
                if (! empty($paths)) {
                    $this->applyNavigationWhereToSql($builder, $whereToApply, $paths, $isOr);
                } else {
                    $this->applyWhere($builder, $whereToApply, $isOr);
                }
            }
    ...
    /**
     * Filter and validate values for IN clause based on property type
     * Removes invalid values like 'undefined' for integer types
     * 
     * @param array $values Array of values (may contain SQL-formatted strings like "'value'" or "123")
     * @param \ReflectionClass $entityReflection Reflection of the entity containing the property
     * @param string $propertyName Name of the property
     * @return array Filtered array of valid SQL-formatted values
     */
    private function filterInvalidValuesForInClause(array $values, \ReflectionClass $entityReflection, string $propertyName): array
    {
        // Try to get the property from the entity
        if (!$entityReflection->hasProperty($propertyName)) {
            // Property not found, return values as-is (let database handle validation)
            return $values;
        }
        ...
        if ($propertyType === null) {
            // Type unknown, return values as-is
            return $values;
        }
        ...
        foreach ($values as $value) {
            ...
            // Remove quotes if present (values come as SQL-formatted strings like "'undefined'" or "3")
            $unquotedValue = trim($value, "'\"");
            $unquotedValue = trim($unquotedValue); // Trim again after removing quotes
            
            // Skip 'undefined', 'null', empty strings, etc. for numeric types
            if (in_array($propertyType, ['int', 'float', 'double'])) {
                // For numeric types, only accept numeric values
                $lowerValue = strtolower($unquotedValue);
                if (in_array($lowerValue, ['undefined', 'null', '', 'nan', 'infinity', '-infinity'])) {
                    log_message('debug', "filterInvalidValuesForInClause - filtering out invalid value '{$unquotedValue}' for {$propertyType} property '{$propertyName}'");
                    continue;
                }
                
                // Check if value is numeric (handles both integer and float strings)
                if (!is_numeric($unquotedValue)) {
                    log_message('debug', "filterInvalidValuesForInClause - filtering out non-numeric value '{$unquotedValue}' for {$propertyType} property '{$propertyName}'");
                    continue;
                }
            }
    ...
    private function applySimpleWhereWithParser($builder, callable|string $predicate, bool $isOr = false): void
    {
        ...
        try {
            $parser = new ExpressionParser($this->entityType, $this->getTableAliasForParser($builder), $this->context);
                    ...
                    if (count($parts) >= 2 && $this->isNavigationTokenCondition($sqlCondition)) {
                        ...
                        if (preg_match('/^([^.]+)\.([^\s]+)\s+(.+)$/', $navExpression, $matches)) {
                            ...
                            if ($navInfo && !$navInfo['isCollection']) {
                                // Ensure JOIN is added (for count() method)
                                ...
                                // Get join alias from requiredJoins (set in buildEfCoreStyleQuery or addJoinForNavigationPath)
                                $joinAlias = null;
                                if (isset($this->requiredJoins[$navigationProperty])) {
                                    $joinAlias = $this->requiredJoins[$navigationProperty]['alias'] ?? null;
                                }
                                
                                // If join alias not found, use table name (for CodeIgniter Query Builder in count() method)
                                // CodeIgniter Query Builder doesn't use aliases by default, so we'll use table name
                                if ($joinAlias === null) {
                    ...
                    if (count($parts) >= 2) {
                        ...
                        if (count($pathParts) === 2) {
                            ...
                            if ($navInfo && !$navInfo['isCollection']) {
                                ...
                                // Build simple IN clause using JOIN alias
                                if ($values !== '?') {
                                    ...
                                    if ($navInfo && $navInfo['isCollection']) {
                                        ...
                                        if ($refNavInfo2 && !$refNavInfo2['isCollection']) {
                                            ...
                                            if ($builder !== null) {
                                                ...
                                                $mainTableAlias = $this->getTableAliasForParser($builder);
                                            }
     ...
     * When a CodeIgniter base builder is used (count/aggregate), the main table has no "u" alias.
     */
    private function getTableAliasForParser($builder = null): string
    {
        if ($builder !== null) {
            return $this->context->getTableName($this->entityType);
        }
    ...
    /**
     * Replace parser alias [u] / u. with the real main table name for CI4 base-builder queries.
     */
    private function normalizeParserAliasForBuilder(string $sqlCondition, $builder): string
    {
        if ($builder === null) {
            return $sqlCondition;
        }
        ...
        return preg_replace('/\bu\./i', $mainTableName . '.', $sqlCondition);
    }
    ...
    private function collectReferenceNavigationPathsFromDotPath(string $fieldPath): array
    {
        ...
        for ($i = 0, $limit = count($parts) - 1; $i < $limit; $i++) {
            ...
            if (! in_array($cumulative, $paths, true)) {
                $paths[] = $cumulative;
            }
    ...
    private function resolvePureReferenceOrderByPath(string $fieldPath): ?array
    {
        ...
        $tableAlias = $joinInfo['alias']
            ?? $this->resolveReferenceNavAliasFromPath($joinKey)
            ?? $joinInfo['table']
            ?? $tableName;
    ...
    private function buildOrderBySqlFromResolvedPath(array $resolved, string $direction, string $alias): ?string
    {
        ...
        $joinAlias = $this->resolveReferenceNavAliasFromPath($joinKey);
        if ($joinAlias !== null) {
            $quotedJoinAlias = $provider->escapeIdentifier($joinAlias);
            $quotedColumn = $provider->escapeIdentifier($columnName);
    ...
    /**
     * buildEfCoreStyleQuery referenceNavIndexes ile aynı kuralda join alias üretir.
     */
    private function resolveReferenceNavAliasFromPath(string $navPath): ?string
    {
        $refIndex = $this->referenceNavIndexes[$navPath] ?? null;
        if ($refIndex === null) {
            return null;
        }
        ...
        return $aliasBase . ($refIndex > 0 ? (string) $refIndex : '');
    }
    ...
    private function expandRawSqlNavigationWhereForCount(string $rawSql, $builder): string
    {
        ...
        if (preg_match_all('/\b([A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)+)\b/', $rawSql, $matches)) {
            // isEmpty/isNotEmpty navigation paths use EXISTS (buildNavigationExistsForEmptyFilter), not JOINs.
        }

        $mainAlias = $this->getTableAliasForParser($builder);
        $refAliasByPath = [];
        foreach ($this->requiredJoins as $path => $info) {
            $refAliasByPath[$path] = $info['alias'] ?? $info['table'];
        }

        $sql = $this->jsonRewriteRawSqlMainTableRefs($rawSql, $mainAlias);
        $sql = $this->rewriteNavigationEmptyNotEmptyConditions($sql, $mainAlias, $refAliasByPath);

        return $this->jsonRewriteRawSqlNavigationRefs($sql, $mainAlias, $refAliasByPath);
    }
    ...
    {
        if (str_contains($navigationProperty, '.')) {
            $parts = explode('.', $navigationProperty);
            $parentEntityType = $this->entityType;
            $parentTableAlias = $this->context->getTableName($this->entityType);
            $cumPath = '';
            foreach ($parts as $part) {
                $cumPath = $cumPath === '' ? $part : $cumPath . '.' . $part;
                if (isset($this->requiredJoins[$cumPath])) {
                    $parentTableAlias = $this->requiredJoins[$cumPath]['alias'] ?? $this->requiredJoins[$cumPath]['table'];
                    $parentEntityType = $this->requiredJoins[$cumPath]['entityType'];
                    continue;
                }
                $this->addJoinFromParentEntity($builder, $part, $parentEntityType, $parentTableAlias, $cumPath);
                if (! isset($this->requiredJoins[$cumPath])) {
                    return;
                }
                $parentTableAlias = $this->requiredJoins[$cumPath]['alias'] ?? $this->requiredJoins[$cumPath]['table'];
    ...
    private function addJoinFromParentEntity(
        ...
        log_message('debug', "addJoinFromParentEntity - '{$joinKey}' on {$parentEntityType}");

        $entityReflection = self::getCachedReflection($parentEntityType);
        if (! $entityReflection->hasProperty($navigationProperty)) {
            log_message('debug', "addJoinFromParentEntity - property '{$navigationProperty}' not found");

            return;
        }

        $navInfo = $this->getNavigationInfoForEntity($navigationProperty, $parentEntityType);
        if ($navInfo === null || $navInfo['isCollection']) {
            return;
        }

        $relatedEntityType = $navInfo['entityType'];
        $foreignKey = $this->getForeignKeyForNavigation($entityReflection, $navigationProperty, false, $parentEntityType);
        ...
        if (! empty($this->includes) && is_array($this->includes)) {
            foreach ($this->includes as $include) {
                ...
                if (($include['path'] ?? null) === $joinKey && isset($include['joinCondition'])) {
    ...
    private function buildEfCoreStyleQuery(): string
    {
        ...
        // Extract collection property WHERE clauses from main WHERE conditions
        // These are WHERE clauses that filter on collection properties (e.g., ReaderAccessCards.ReaderID)
        $collectionWhereClauses = [];
        foreach ($this->wheres as $whereIndex => $whereItem) {
            if (is_array($whereItem) && isset($whereItem['predicate'])) {
                $where = $whereItem['predicate'];
            } else {
                $where = $whereItem;
            }
            ...
            // Detect navigation paths in WHERE clause
            $paths = $this->detectNavigationPaths($where);
            foreach ($paths as $navPath) {
                $navInfo = $this->getNavigationInfo($navPath);
                if ($navInfo && $navInfo['isCollection']) {
                    // This WHERE clause filters on a collection property
                    // Parse the predicate to extract the condition for collection subquery
                    if (isset($navigationFilters[$whereIndex])) {
                        try {
                            $parser = new ExpressionParser($this->entityType, '{alias}', $this->context);
                            $reflection = new \ReflectionFunction($where);
                            $staticVariables = $reflection->getStaticVariables();
                            ...
                            // Try to extract variables from closure code
                            $closureFile = $reflection->getFileName();
                            $closureStartLine = $reflection->getStartLine();
                            $closureEndLine = $reflection->getEndLine();
                            ...
                            if ($closureFile && file_exists($closureFile) && $closureStartLine && $closureEndLine) {
                                ...
                                if (preg_match_all('/=>\s*.+?\$([a-zA-Z_][a-zA-Z0-9_]*)/', $closureCode, $varMatches)) {
                                    foreach ($varMatches[1] as $possibleVarName) {
                                        if (strlen($possibleVarName) > 1 || !in_array($possibleVarName, ['e', 'u', 'x', 'a', 'i', 'o', 'p', 'c'])) {
                                            if (!in_array($possibleVarName, $useVarNames)) {
                                                $useVarNames[] = $possibleVarName;
                                            }
                                        }
                            ...
                            if ($sqlCondition && strpos($sqlCondition, 'NAVIGATION:') !== false) {
                                ...
                                if (preg_match('/NAVIGATION:([^\s]+)\s+(.+)$/', $sqlCondition, $navMatches)) {
                                    $navPathWithProperty = $navMatches[1]; // e.g., "ReaderAccessCards.ReaderID"
                                    $sqlOperator = $navMatches[2]; // e.g., "= 86"
                                    
                                    $pathParts = explode('.', $navPathWithProperty);
                                    if (count($pathParts) === 2 && $pathParts[0] === $navPath) {
                                        // Extract property name
                                        $propertyName = $pathParts[1]; // e.g., "ReaderID"
                                        
                                        // Get column name from collection entity
                                        $collectionEntityReflection = new ReflectionClass($navInfo['entityType']);
                                        $columnName = $this->getColumnNameFromProperty($collectionEntityReflection, $propertyName);
                                        
                                        // Build collection subquery WHERE clause condition
                                        // Format: {alias}.ColumnName = value (using {alias} placeholder)
                                        $provider = \Yakupeyisan\CodeIgniter4\EntityFramework\Providers\DatabaseProviderFactory::getProvider($this->connection);
                                        $quotedColumnName = $provider->escapeIdentifier($columnName);
                                        $collectionCondition = "{alias}.{$quotedColumnName} {$sqlOperator}";
                                        
                                        if (!isset($collectionWhereClauses[$navPath])) {
                                            $collectionWhereClauses[$navPath] = [];
                                        }
                                        $collectionWhereClauses[$navPath][] = $collectionCondition;
                                        log_message('debug', "buildEfCoreStyleQuery: Extracted collection WHERE clause for '{$navPath}': {$collectionCondition}");
                                    }
                                }
                            }
                        } catch (\Exception $e) {
                            log_message('error', "buildEfCoreStyleQuery: Failed to extract collection WHERE clause for '{$navPath}': " . $e->getMessage());
                        }
                    }
                }
            }
        ...
        foreach ($collectionIncludes as $collectionIncludeIdx => $collectionInclude) {
            ...
            // Add WHERE clause from main WHERE conditions if collection property was filtered
            if (isset($collectionWhereClauses[$navPath]) && !empty($collectionWhereClauses[$navPath])) {
                $collectionConditions = $collectionWhereClauses[$navPath];
                $additionalWhereClause = implode(' AND ', $collectionConditions);
                if ($whereClause) {
                    $whereClause = "({$whereClause}) AND ({$additionalWhereClause})";
                } else {
                    $whereClause = $additionalWhereClause;
                }
                log_message('debug', "buildEfCoreStyleQuery: Added WHERE clause from main WHERE conditions for collection '{$navPath}': {$whereClause}");
            }
            ...
            if ($navInfo && $navInfo['isCollection']) {
                ...
                $this->collectionNavIndexes[$navPath] = $collectionIndex;
                log_message('debug', "buildEfCoreStyleQuery: Stored collection index {$collectionIndex} for '{$navPath}'");
        ...
        foreach ($collectionSubqueries as $idx => $subquery) {
            ...
            if ($joinEntityType) {
                ...
                if (!empty($parentPath) && $parentPath !== $navPath) {
                    // This collection is nested under a reference navigation or a collection with references
                    // Handle composite parent paths (e.g., "EmployeeAccessGroups.AccessGroup")
                    $parentParts = explode('.', $parentPath);
                    
                    // Check if parent path contains a collection navigation
                    // If so, we need to join to the collection subquery, not the main entity
                    $parentIsInCollectionSubquery = false;
                    $parentCollectionSubqueryAlias = null;
                    $parentReferenceNavName = null;
                    
                    if (count($parentParts) > 1) {
                        // Multi-level parent path - check if first part is a collection
                        $firstParentPart = $parentParts[0];
                        $lastParentPart = $parentParts[count($parentParts) - 1];
                        
                        // Check if first part is a collection navigation
                        $firstPartNavInfo = $this->getNavigationInfo($firstParentPart);
                    ...
                    if ($parentIsInCollectionSubquery && $parentCollectionSubqueryAlias !== null && $parentReferenceNavName !== null) {
                        // Parent is a reference navigation within a collection subquery
                        // Join to the collection subquery's reference navigation primary key
                        // The column will be named like IdAccessGroup0 (Id + EntityShortName + 0)
                        log_message('debug', "buildEfCoreStyleQuery: Collection '{$navPath}' parent '{$parentPath}' is within collection subquery '{$parentCollectionSubqueryAlias}', reference nav: '{$parentReferenceNavName}'");
                        
                        // Get the entity type for the parent reference navigation to determine the correct column name
                        // We need to get the navigation info from the collection entity type, not the main entity
                        $parentCollectionEntityType = null;
                        foreach ($collectionSubqueries as $cs) {
                            if ($cs['navigation'] === $firstParentPart) {
                                $parentCollectionEntityType = $cs['entityType'];
                                break;
                            }
                        }
                        
                        if ($parentCollectionEntityType !== null) {
                            $parentRefNavInfo = $this->getNavigationInfoForEntity($parentReferenceNavName, $parentCollectionEntityType);
                            if ($parentRefNavInfo && !$parentRefNavInfo['isCollection']) {
                                $parentRefEntityReflection = new ReflectionClass($parentRefNavInfo['entityType']);
                                $parentRefEntityShortName = $parentRefEntityReflection->getShortName();
                                
                                // Build the parent column name: Id{EntityShortName}0 (e.g., IdAccessGroup0)
                                $parentPkColumnName = "Id{$parentRefEntityShortName}0";
                                
                                log_message('debug', "buildEfCoreStyleQuery: Using collection subquery column '[{$parentCollectionSubqueryAlias}].[{$parentPkColumnName}]' for join");
                                $joinCondition = "[{$parentCollectionSubqueryAlias}].[{$parentPkColumnName}] = [{$collectionSubqueryAlias}].[{$fkColumnForJoin}]";
                            } else {
                                log_message('warning', "buildEfCoreStyleQuery: Could not resolve parent reference navigation '{$parentReferenceNavName}' in collection entity '{$parentCollectionEntityType}', falling back to main entity");
                                $joinCondition = "[{$subqueryAlias}].[{$mainPrimaryKeyColumn}] = [{$collectionSubqueryAlias}].[{$fkColumnForJoin}]";
                            }
                        } else {
                            log_message('warning', "buildEfCoreStyleQuery: Could not find parent collection entity type for '{$firstParentPart}', falling back to main entity");
                            $joinCondition = "[{$subqueryAlias}].[{$mainPrimaryKeyColumn}] = [{$collectionSubqueryAlias}].[{$fkColumnForJoin}]";
                        }
                    } else {
                        // Single-level parent path - join to main entity's reference navigation
                        $parentNavInfo = $this->getNavigationInfo($parentPath);
        ...
        if (!empty($this->orderBys)) {
            ...
            foreach ($this->orderBys as $orderBy) {
                ...
                if (isset($staticVariables['field']) && is_string($staticVariables['field']) && strpos($staticVariables['field'], '.') !== false) {
                    ...
                    if (!$isCollectionNav && $firstNavProp !== null) {
                        // Paths like Employee.SurName on EmployeeAccessDoor: Employee is a main-entity
                        // reference navigation and SurName is a scalar column on Employee (already in main subquery).
                        // Do not route these through a collection subquery alias (e.g. s0).
                        $skipCase2dForMainReferenceLeaf = false;
                        $mainRefNavInfo = $this->getNavigationInfo($firstNavProp);
                        if ($mainRefNavInfo && ! $mainRefNavInfo['isCollection']
                            && $nestedRest !== ''
                            && ! str_contains($nestedRest, '.')) {
                            $mainRefEntityReflection = new \ReflectionClass($mainRefNavInfo['entityType']);
                            if ($mainRefEntityReflection->hasProperty($nestedRest)) {
                                $skipCase2dForMainReferenceLeaf = true;
                            }
                        }

                        if (! $skipCase2dForMainReferenceLeaf) {
                        foreach ($collectionSubqueries as $idx => $subquery) {
                ...
                if ($isCollectionNav && $collectionSubqueryAlias && $collectionColumnName) {
                    // Safety: paths like Employee.SurName belong to a main-entity reference nav column
                    // (aliased in main subquery as SurName0), not a collection subquery column.
                    if (isset($staticVariables['field']) && is_string($staticVariables['field']) && str_contains($staticVariables['field'], '.')) {
                        $fieldParts = explode('.', $staticVariables['field'], 2);
                        if (count($fieldParts) === 2) {
                            $mainRefNavInfo = $this->getNavigationInfo($fieldParts[0]);
                            if ($mainRefNavInfo && ! $mainRefNavInfo['isCollection']) {
                                $leafReflection = new \ReflectionClass($mainRefNavInfo['entityType']);
                                if ($leafReflection->hasProperty($fieldParts[1])) {
                                    $isCollectionNav = false;
                                }
                            }
                        }
    ...
    /**
     * Parse nested collection info recursively for parsing
     */
    private function parseNestedCollectionInfo(string $parentPath, array $thenIncludes, array &$collectionNavInfo, int &$collectionIndex): void
    {
        foreach ($thenIncludes as $thenInclude) {
            $thenIncludeNav = is_string($thenInclude) ? $thenInclude : ($thenInclude['navigation'] ?? $thenInclude);
            $thenIncludePath = $parentPath . '.' . $thenIncludeNav;
            
            log_message('debug', "parseNestedCollectionInfo: Checking thenInclude '{$thenIncludeNav}', full path: '{$thenIncludePath}'");
            
            $thenNavInfo = $this->getNavigationInfo($thenIncludePath);
            if ($thenNavInfo && $thenNavInfo['isCollection']) {
                // Nested collection found
                $storedIndex = $this->collectionNavIndexes[$thenIncludePath] ?? null;
                $indexToUse = $storedIndex !== null ? $storedIndex : $collectionIndex;
                
                $nestedThenIncludes = [];
                if (is_array($thenInclude) && isset($thenInclude['thenIncludes'])) {
                    $nestedThenIncludes = $thenInclude['thenIncludes'];
                }
                
                $joinEntityType = $thenNavInfo['joinEntityType'];
                if ($joinEntityType !== null) {
                    $actualRelatedEntityType = $this->findRelatedEntityFromJoinEntity($joinEntityType, $thenIncludePath);
                    if ($actualRelatedEntityType === null) {
                        $actualRelatedEntityType = $thenNavInfo['entityType'];
                    }
                } else {
                    $actualRelatedEntityType = $thenNavInfo['entityType'];
                }
                
                $collectionNavInfo[$thenIncludePath] = [
                    'index' => $indexToUse,
                    'joinEntityType' => $joinEntityType,
                    'entityType' => $actualRelatedEntityType,
                    'thenIncludes' => $nestedThenIncludes,
                    'parentPath' => $parentPath
                ];
                ...
                log_message('debug', "parseNestedCollectionInfo: Added nested collection '{$thenIncludePath}' with parent '{$parentPath}' at index {$indexToUse}");
            } elseif ($thenNavInfo && !$thenNavInfo['isCollection']) {
                // This is a reference navigation, check its thenIncludes recursively
                if (is_array($thenInclude) && isset($thenInclude['thenIncludes'])) {
                    $this->parseNestedCollectionInfo($thenIncludePath, $thenInclude['thenIncludes'], $collectionNavInfo, $collectionIndex);
                }
            }
        }
    }
    ...
    private function getNavigationInfo(string $navigationProperty): ?array
    {
        ...
        if ($isCollection) {
            ...
            // First, try to get FK from the related entity properties with ForeignKey attribute
            $relatedEntityReflection = new ReflectionClass($relatedEntityType);
            $mainEntityShortName = $entityReflection->getShortName();
            
            // Check if there's a ForeignKey attribute pointing to the main entity
            $foundFk = false;
            foreach ($relatedEntityReflection->getProperties() as $property) {
            ...
            // If not found via ForeignKey attribute, try convention-based property name matching
            if (!$foundFk) {
                // Try common patterns: MainEntityName + "Id", or check if property exists
                $conventionFk = $mainEntityShortName . 'Id';
                if ($relatedEntityReflection->hasProperty($conventionFk)) {
                    $foreignKey = $conventionFk;
                    log_message('debug', "getNavigationInfo: Using convention FK '{$foreignKey}' for '{$navigationProperty}'");
                } else {
                    // Try alternative: check for properties ending with "Id" that might reference the main entity
                    // Look for properties that match the main entity's primary key column name pattern
                    $mainPkColumn = $this->getPrimaryKeyColumnName($entityReflection);
                    // If main entity uses non-standard PK (like ReaderID), check if related entity has matching property
                    foreach ($relatedEntityReflection->getProperties() as $property) {
                        $colAttributes = $property->getAttributes(\Yakupeyisan\CodeIgniter4\EntityFramework\Attributes\Column::class);
                        if (!empty($colAttributes)) {
                            $colAttr = $colAttributes[0]->newInstance();
                            $colName = $colAttr->name ?? $property->getName();
                            // Check if this column name matches the main entity's primary key column
                            if (strcasecmp($colName, $mainPkColumn) === 0) {
                                $foreignKey = $property->getName();
                                $foundFk = true;
                                log_message('debug', "getNavigationInfo: Found FK property '{$foreignKey}' matching main entity PK column '{$mainPkColumn}' for '{$navigationProperty}'");
                                break;
                            }
                        }
                    }
                    ...
                    if (!$foundFk) {
                        $foreignKey = $conventionFk;
                        log_message('debug', "getNavigationInfo: Using convention FK '{$foreignKey}' as fallback for '{$navigationProperty}'");
                    }
                }
            }
        } else {
    ...
    private function buildCollectionSubquery(string $navPath, array $navInfo, int $index, array $thenIncludes = [], int &$nestedSubqueryIndex = 1, ?string $whereClause = null): ?array
    {
        ...
        if ($whereClause !== null && trim($whereClause) !== '') {
            ...
            // Escape the alias properly using provider
            $quotedRelatedAlias = $provider->escapeIdentifier($relatedAlias);
            // Replace {alias} with escaped alias
            $whereClause = str_replace('{alias}', $quotedRelatedAlias, $whereClause);
    ...
    private function convertNavigationWhereToSql(callable $predicate, array $navigationPaths, array $referenceNavAliases = []): ?string
    {
        ...
        try {
            ...
            if ($sqlCondition && $this->isNavigationTokenCondition($sqlCondition)) {
                $mainAlias = $this->getTableAliasForParser();
                $expandedNav = $this->expandNavigationTokenForRawSql(trim($sqlCondition), $mainAlias, $referenceNavAliases);
                if ($expandedNav !== null) {
            ...
            if ($sqlCondition) {
                ...
                if (strpos($sqlCondition, 'NAVIGATION:') !== false) {
                    ...
                    if (preg_match('/NAVIGATION:([^\s]+)\s+(.+)$/', $sqlCondition, $navMatches)) {
                        ...
                        if (count($pathParts) === 2) {
                            $navigationProperty = $pathParts[0]; // e.g., "ReaderAccessCards" or "CustomField"
                            $columnName = $pathParts[1]; // e.g., "ReaderID" or "CustomField01"
                            
                            // Get navigation info for navigation property
                            $navInfo = $this->getNavigationInfo($navigationProperty);
                            if ($navInfo) {
                                if ($navInfo['isCollection']) {
                                    // Collection property - build EXISTS subquery
                                    ...
                                    // Build EXISTS subquery
                                    $sqlCondition = "EXISTS (SELECT 1 FROM {$quotedCollectionTable} WHERE {$quotedCollectionTable}.{$quotedCollectionFk} = {$quotedMainAlias}.{$quotedMainPk} AND {$quotedCollectionTable}.{$quotedFilterColumn} {$sqlOperator})";
                                    
                                    log_message('debug', 'convertNavigationWhereToSql - converted NAVIGATION: prefix to EXISTS subquery: ' . $sqlCondition);
                                } else {
                                    // Reference navigation property - use JOIN alias
                                    $alias = $referenceNavAliases[$navigationProperty] ?? $this->getTableAlias($navigationProperty, 0);
                                    $refEntityReflection = new ReflectionClass($navInfo['entityType']);
                                    $refColumnName = $this->getColumnNameFromProperty($refEntityReflection, $columnName);
                                    $provider = \Yakupeyisan\CodeIgniter4\EntityFramework\Providers\DatabaseProviderFactory::getProvider($this->connection);
                                    $quotedAlias = $provider->escapeIdentifier($alias);
                                    ...
                                    // Build SQL condition using JOIN alias
                                    $sqlCondition = "{$quotedAlias}.{$quotedRefColumn} {$sqlOperator}";
    ...
    private function expandNavigationInForRawSql(string $parsedSql, string $mainAlias): ?string
    {
        ...
        if (count($pathParts) === 3) {
            ...
            if ($firstNavInfo !== null && $firstNavInfo['isCollection']) {
                ...
                if ($secondNavInfo !== null && ! $secondNavInfo['isCollection']) {
                    ...
                    if ($leafReflection->hasProperty($pathParts[2])) {
                        $recursiveSql = $this->buildNavigationPathConditionRecursive($pathParts, $values, null, $mainAlias);
            ...
            // e.g. Employee.EmployeeDepartments.DepartmentID — filter on many-to-many / junction entity row (exactly 3 parts)
            if ($nestedNavInfo && $nestedNavInfo['isCollection'] && count($pathParts) === 3) {
                ...
                $junctionProp = $collectionEntityReflection->getProperty($pathParts[2]);
                $junctionType = $junctionProp->getType();
                if ($junctionType !== null && !$junctionType->isBuiltin()) {
            ...
            $existsSql = $this->buildNestedReferenceNavigationExistsForFilter($pathParts, "IN ({$values})", $mainAlias);
            if ($existsSql !== null) {
                return $existsSql;
            }
     ...
     */
    private function expandNavigationTokenForRawSql(string $sqlCondition, string $mainAlias, array $referenceNavAliases = []): ?string
    {
        ...
        $sqlOperator = trim($navMatches[2]);
        $resolved = $this->resolveNavigationTokenSql($navPath, $sqlOperator, $mainAlias, $referenceNavAliases);
        if ($resolved !== null) {
        ...
        if (count($pathParts) >= 4) {
            ...
            if ($recursiveSql !== null) {
                $op = trim($sqlOperator);
                if (preg_match('/^IS\s+(NOT\s+)?NULL$/i', $op)) {
                    $expanded = preg_replace('/\s+IN\s*\(\?\)\s*\)\s*$/i', ' AND ' . $this->extractNavigationLeafColumnFromExists($recursiveSql) . ' ' . $op . ')', $recursiveSql);
                    if ($expanded !== null && $expanded !== $recursiveSql) {
                        return $isNotWrapped ? "NOT ({$expanded})" : $expanded;
                    }
                }
                $expanded = preg_replace('/\s+IN\s*\(\?\)\s*\)\s*$/i', ' AND ' . $this->extractNavigationLeafColumnFromExists($recursiveSql) . ' ' . $sqlOperator . ')', $recursiveSql);
                if ($expanded !== null && $expanded !== $recursiveSql) {
                    return $isNotWrapped ? "NOT ({$expanded})" : $expanded;
                }
            }
        ...
        // 3 parts: Collection.Reference.Column (e.g. EmployeeDepartments.Department.DepartmentID)
        if (count($pathParts) === 3) {
            $collNavInfo = $this->getNavigationInfo($pathParts[0]);
            if ($collNavInfo && $collNavInfo['isCollection']) {
                $refNavInfo = $this->getNavigationInfoForEntity($pathParts[1], $collNavInfo['entityType']);
                if ($refNavInfo && !$refNavInfo['isCollection']) {
                    $leafReflection = new \ReflectionClass($refNavInfo['entityType']);
                    if ($leafReflection->hasProperty($pathParts[2])) {
                        $recursiveSql = $this->buildNavigationPathConditionRecursive($pathParts, '?', null, $mainAlias);
                        if ($recursiveSql !== null) {
                            $op = trim($sqlOperator);
        ...
        // 3+ parts: reference.collection... or reference.reference.column
        if (count($pathParts) >= 3) {
            $refNavInfo = $this->getNavigationInfo($pathParts[0]);
            if ($refNavInfo && !$refNavInfo['isCollection']) {
                $nestedNavInfo = $this->getNavigationInfoForEntity($pathParts[1], $refNavInfo['entityType']);
                if ($nestedNavInfo && $nestedNavInfo['isCollection'] && isset($pathParts[2]) && count($pathParts) === 3) {
                    // collection scalar column on junction entity (exactly 3 parts)
                    $collectionEntityReflection = new \ReflectionClass($nestedNavInfo['entityType']);
                    if ($collectionEntityReflection->hasProperty($pathParts[2])) {
                        $junctionProp = $collectionEntityReflection->getProperty($pathParts[2]);
                        $junctionType = $junctionProp->getType();
                        $isNavProp = $junctionType !== null && !$junctionType->isBuiltin();
                        if (!$isNavProp) {
                        $mainEntityReflection = new \ReflectionClass($this->entityType);
                        $mainFkColumn = $this->getColumnNameFromProperty($mainEntityReflection, $refNavInfo['foreignKey']);
                        $collFkColumn = $this->getColumnNameFromProperty($collectionEntityReflection, $nestedNavInfo['foreignKey']);
                    ...
                    $expanded = $this->buildNestedReferenceNavigationExistsForFilter($pathParts, $sqlOperator, $mainAlias);
                }
            }
        }
    ...
    private function convertSimpleWhereToSql(callable $predicate, string $alias, array $referenceNavAliases = []): ?string
    {
        try {
            ...
            if (!empty($sqlCondition) && $this->isNavigationTokenCondition($sqlCondition)) {
                $expandedNav = $this->expandNavigationTokenForRawSql(trim($sqlCondition), $alias, $referenceNavAliases);
                if ($expandedNav !== null) {
                    return $expandedNav;
                }
            }
            ...
            if (!empty($sqlCondition)) {
                ...
                if (strpos($sqlCondition, 'NAVIGATION:') === 0) {
                    ...
                    if (count($parts) >= 2) {
                        ...
                        if (preg_match('/^([^.]+)\.([^\s]+)\s+(.+)$/', $navExpression, $matches)) {
                            ...
                            if ($navInfo && !$navInfo['isCollection']) {
                                // Reference navigation property - use JOIN alias
                                // Get join alias from referenceNavAliases (passed from buildEfCoreStyleQuery)
                                $joinAlias = $referenceNavAliases[$navigationProperty] ?? null;
                                
                                // If join alias not found, try to find it from requiredJoins
                                if ($joinAlias === null && isset($this->requiredJoins[$navigationProperty])) {
                                    $joinAlias = $this->requiredJoins[$navigationProperty]['alias'] ?? null;
                                }
                                
                                // If join alias still not found, try to generate alias based on navigation property name
                                // This is a fallback - ideally joinAlias should be set in referenceNavAliases or requiredJoins
                                if ($joinAlias === null) {
                ...
                elseif (strpos($sqlCondition, 'NAVIGATION_IN:') === 0) {
                    ...
                    if (count($parts) >= 2) {
                        ...
                        // If values string is empty (all values were filtered out), return false condition
                        if ($values === '') {
                            log_message('debug', "convertSimpleWhereToSql - values string is empty after filtering, returning false condition");
                            return "1=0";
                        }
                            ...
                            if ($navInfo && !$navInfo['isCollection']) {
                                ...
                                if ($values !== '?') {
                                    ...
                                    if (empty($filteredValues)) {
                                        // All values were invalid, return a condition that always evaluates to false
    ...
    private function parseEfCoreStyleResults(array $results): array
    {
        ...
        foreach ($this->wheres as $whereItem) {
            $where = is_array($whereItem) ? $whereItem['predicate'] : $whereItem;
            // Skip if predicate is null or not callable
            if ($where === null || !is_callable($where)) {
                continue;
            }
            $paths = $this->detectNavigationPaths($where);
        ...
        // Get collection navigation info (including nested collections from thenIncludes)
        $collectionNavInfo = [];
        $collectionIndex = 0;
        ...
        foreach ($this->includes as $include) {
            ...
            if ($navInfo && $navInfo['isCollection']) {
                ...
                $thenIncludes = [];
                foreach ($this->includes as $inc2) {
                    $includeNavPath = $inc2['path'] ?? $inc2['navigation'] ?? null;
                    if ($includeNavPath === $navPath && isset($inc2['thenIncludes'])) {
                        $thenIncludes = $inc2['thenIncludes'];
                        break;
                    }
                }
                
                // Get the actual related entity type
                $joinEntityType = $navInfo['joinEntityType'];
                if ($joinEntityType !== null) {
            ...
            // Check thenIncludes for nested collections recursively
            if ($navInfo && !$navInfo['isCollection'] && isset($include['thenIncludes'])) {
                $this->parseNestedCollectionInfo($navPath, $include['thenIncludes'], $collectionNavInfo, $collectionIndex);
            }
        }
        
        log_message('debug', "parseEfCoreStyleResults: Collection navigation info: " . implode(', ', array_keys($collectionNavInfo)));
        ...
        foreach ($results as $row) {
            ...
            if (!isset($entitiesMap[$entityId])) {
                ...
                foreach ($collectionNavInfo as $navPath => $info) {
                    // For nested collections, only initialize if it's top-level
                    // Nested collections will be initialized on their parent entity
                    if (str_contains($navPath, '.')) {
            ...
            // Parse collection navigation properties (both top-level and nested)
            foreach ($collectionNavInfo as $navPath => $info) {
                log_message('debug', "parseEfCoreStyleResults: Processing collection '{$navPath}' for entity ID {$entityId}");
                
                // Determine if this is a nested collection (e.g., Card.Employee.EmployeeDepartments)
                $isNested = str_contains($navPath, '.');
                $targetEntity = $entity;
                $targetEntityReflection = $entityReflection;
                $parentIdValue = $entityId;
                
                if ($isNested) {
                    // Navigate to parent entity
                    $pathParts = explode('.', $navPath);
                    $collectionPropertyName = array_pop($pathParts);
                    $parentPath = implode('.', $pathParts);
                    
                    // Get parent entity by traversing path
                    $currentEntity = $entity;
                    $currentEntityType = $this->entityType;
                    
                    foreach ($pathParts as $part) {
                        $currentReflection = new ReflectionClass($currentEntityType);
                        if (!$currentReflection->hasProperty($part)) {
                            log_message('debug', "parseEfCoreStyleResults: Parent property '{$part}' not found for nested collection '{$navPath}'");
                            $currentEntity = null;
                            break;
                        }
                        ...
                        if ($currentEntity === null) {
                            log_message('debug', "parseEfCoreStyleResults: Parent entity is null at '{$part}' for nested collection '{$navPath}'");
                            break;
                        }
                        
                        $currentEntityType = get_class($currentEntity);
                    }
                    ...
                    $targetEntityReflection = new ReflectionClass($currentEntityType);
                    
                    // Get parent ID from row
                    if (isset($referenceNavInfo[$parentPath])) {
                        $parentIndex = $referenceNavInfo[$parentPath]['index'];
                        $parentIdKey = 's_Id' . $parentIndex;
                        $parentIdValue = $row[$parentIdKey] ?? null;
                        log_message('debug', "parseEfCoreStyleResults: Nested collection '{$navPath}' parent '{$parentPath}' ID: " . ($parentIdValue ?? 'null'));
                    }
                    
                    if ($parentIdValue === null) {
                        log_message('debug', "parseEfCoreStyleResults: Parent ID is null for nested collection '{$navPath}', skipping");
                        continue;
                    }
                    
                    // Get collection property from target entity
                    if (!$targetEntityReflection->hasProperty($collectionPropertyName)) {
                        log_message('debug', "parseEfCoreStyleResults: Collection property '{$collectionPropertyName}' not found in parent entity");
                        continue;
                    }
                    
                    $navProperty = $targetEntityReflection->getProperty($collectionPropertyName);
                } else {
                    // Top-level collection
                    if (!$entityReflection->hasProperty($navPath)) {
                        log_message('debug', "parseEfCoreStyleResults: Collection property '{$navPath}' not found in main entity");
                        continue;
                    }
                    $navProperty = $entityReflection->getProperty($navPath);
                }
                ...
                if ($collectionId !== null) {
                    // For nested collections, verify FK matches parent ID
                    if ($isNested && isset($info['parentPath']) && $parentIdValue !== null) {
                        $relatedEntityReflection = $reflectionCache[$info['entityType']] ?? new ReflectionClass($info['entityType']);
                        if (isset($referenceNavInfo[$info['parentPath']])) {
                            $parentEntityType = $referenceNavInfo[$info['parentPath']]['entityType'];
                            $parentEntityShortName = (new ReflectionClass($parentEntityType))->getShortName();
                            $expectedFkName = $parentEntityShortName . 'ID'; // e.g., EmployeeID
                            
                            // Check if FK column exists
                            if ($relatedEntityReflection->hasProperty($expectedFkName)) {
                                $fkColumnName = $this->getColumnNameFromProperty($relatedEntityReflection, $expectedFkName);
                                $fkKey = $subqueryAlias . '_' . $fkColumnName;
                                $fkValue = $row[$fkKey] ?? null;
                                
                                if ($fkValue !== null && $fkValue != $parentIdValue) {
                                    log_message('debug', "parseEfCoreStyleResults: Skipping collection item for '{$navPath}': FK {$fkValue} != parent ID {$parentIdValue}");
                                    continue;
                                }
                            }
                        }
                    ...
                    if (!empty($collection)) {
                        ...
                        $mapKey = ($parentIdValue ?? $entityId) . '_' . $navPath;
        ...
        foreach ($entitiesMap as $entityId => $entity) {
            foreach ($collectionNavInfo as $navPath => $info) {
                // Skip nested collections in debug (they're on parent entities, not main entity)
                if (str_contains($navPath, '.')) {
                    continue;
                }
                if ($entityReflection->hasProperty($navPath)) {
    ...
    /**
     * EXISTS fallback when COUNT/CI builder has no JOIN alias for a reference navigation filter.
     */
    private function buildReferenceNavigationExistsForFilter(
        string $navigationProperty,
        string $columnName,
        string $sqlOperator,
        string $mainAlias
        ...
        $provider = \Yakupeyisan\CodeIgniter4\EntityFramework\Providers\DatabaseProviderFactory::getProvider($this->connection);
        $quotedMainAlias = $provider->escapeIdentifier($mainAlias);
        $mainReflection = new \ReflectionClass($this->entityType);
        $foreignKey = $this->getForeignKeyForNavigation($mainReflection, $navigationProperty, false, $this->entityType);
        $relatedReflection = new \ReflectionClass($navInfo['entityType']);
        $relatedTable = $this->context->getTableName($navInfo['entityType']);
        $refColumn = $this->getColumnNameFromProperty($relatedReflection, $columnName);
        ...
        if ($customJoin !== '') {
            ...
            $fkOnRelated = $this->getColumnNameFromProperty($relatedReflection, $foreignKey);
            $mainPk = $this->getPrimaryKeyColumnName($mainReflection);
            $quotedFkOnRelated = $provider->escapeIdentifier($fkOnRelated);
            $quotedMainPk = $provider->escapeIdentifier($mainPk);
            $joinOn = "rel.{$quotedFkOnRelated} = {$quotedMainAlias}.{$quotedMainPk}";
        }

        return "EXISTS (SELECT 1 FROM {$quotedRelatedTable} AS rel WHERE {$joinOn} AND rel.{$quotedRefColumn} {$sqlOperator})";
    }

    /**
     * EXISTS for reference.reference.column filters (e.g. Employee.CustomField.CustomField07).
     * Reuses buildJoinCondition so inverse 1:1 navigations (CustomField.EmployeeID) join correctly.
     *
     * @param string[] $pathParts [referenceNav, nestedRefNav, columnName]
     */
    private function buildNestedReferenceNavigationExistsForFilter(
        array $pathParts,
        string $sqlOperator,
        string $mainAlias
    ): ?string {
        ...
        [$referenceNavProperty, $nestedRefProperty, $columnName] = $pathParts;
        $refNavInfo = $this->getNavigationInfo($referenceNavProperty);
    ...
    private function resolveNavigationTokenSql(
        ...
        if (count($pathParts) === 2) {
            ...
            // CI base-builder COUNT: EXISTS avoids broken cross-catalog JOIN aliases (e.g. Card → Employee filters).
            if ($mainAlias !== 'u') {
    ...
    /**
     * Convert NAVIGATION:path OPERATOR to executable SQL (count builder and EF-style queries).
     *
     * @param array<string, string> $referenceNavAliases Join aliases from buildEfCoreStyleQuery
     */
    private function convertNavigationConditionToSql(
        string $navPath,
        string $sqlOperator,
        $builder = null,
        array $referenceNavAliases = []
        ...
        $pathParts = $this->stripRedundantRootEntityFromPathParts(explode('.', $navPath));
        if (count($pathParts) === 3) {
            $nestedRefExists = $this->buildNestedReferenceNavigationExistsForFilter($pathParts, $sqlOperator, $mainAlias);
            if ($nestedRefExists !== null) {
                return $nestedRefExists;
            }
        }
        if (count($pathParts) >= 3) {
            return $this->buildCollectionReferenceExistsCondition($pathParts, $sqlOperator, $mainAlias);
        }
        ...
        if (count($pathParts) === 2) {
            ...
            if ($navInfo['isCollection']) {
                if ($builder !== null) {
                    ...
                    return "{$quotedCollectionTable}.{$quotedFilterColumn} {$sqlOperator}";
                }

                return $this->resolveNavigationTokenSql($navPath, $sqlOperator, $mainAlias, $referenceNavAliases);
            }
    ...
    /**
     * EXISTS subquery for Collection.Reference.Column filters (e.g. EmployeeAccessGroups.AccessGroup.AccessGroupName).
     *
     * @param string[] $pathParts [collectionNav, referenceNav, columnName, ...]
     */
    private function buildCollectionReferenceExistsCondition(array $pathParts, string $sqlOperator, string $mainTableAlias): ?string
    {
        if (count($pathParts) < 3) {
            return null;
        }
        ...
        $collectionAlias = strtolower(substr($collectionProperty, 0, 2));
        $refAlias = strtolower(substr($referenceProperty, 0, 1));
        $quotedCollectionAlias = $provider->escapeIdentifier($collectionAlias);
        $quotedRefAlias = $provider->escapeIdentifier($refAlias);
     ...
     */
    private function buildNavigationPathConditionRecursive(array $pathParts, string $values, $builder = null, ?string $mainAlias = null): ?string
    {
        if (count($pathParts) < 2) {
        ...
        return $this->buildNavigationPathConditionRecursiveInternal($pathParts, $columnName, $values, $builder, $this->entityType, 0, $mainAlias);
    }

    /**
     * EXISTS alt sorgusundaki son kolon ifadesini çıkarır (IN (?) öncesindeki parça).
     */
    private function extractNavigationLeafColumnFromExists(string $existsSql): string
    {
        if (preg_match('/\s+AND\s+(.+?)\s+IN\s*\(\?\)\s*\)\s*$/i', $existsSql, $m)) {
            return trim($m[1]);
        }
    ...
    /**
     * JSON / raw SQL bağlamında ana tablo takma adını çözümler.
     */
    private function resolveMainTableAliasForNavigation($builder, ?string $mainAlias, string $fallbackEntityType): string
    {
        if ($mainAlias !== null && $mainAlias !== '') {
            return $mainAlias;
        }
        if ($builder !== null) {
            return $this->context->getTableName($fallbackEntityType);
        }

        return $this->getTableAliasForParser($builder);
    }
    ...
    private function buildNavigationPathConditionRecursiveInternal(
        ...
        if ($navInfo['isCollection']) {
            ...
            if (!empty($pathParts)) {
                ...
                $mainTableAlias = $this->resolveMainTableAliasForNavigation($builder, $mainAlias, $rootEntityType);
    ...
    private function buildCollectionExistsWithRecursivePath(
        ...
        $mainTableAlias = $this->resolveMainTableAliasForNavigation($builder, $mainAlias, $mainEntityForExists);
